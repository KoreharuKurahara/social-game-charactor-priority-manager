...
**ユーザーストーリー:** プレイヤーとして、キャラクター詳細画面で所持・未所持ステータスを確認し、更新したい。これにより、新しく取得したキャラクターを即座に管理対象から除外できる。

#### 受入基準

1. WHEN キャラクター一覧を表示する時 THEN システムは各キャラクターの所持ステータス（所持済み/未所持）を視覚的に表示する
2. WHEN キャラクターカードをクリックした時 THEN システムはキャラクター詳細画面を全面表示する
3. WHEN キャラクター詳細画面を表示する時 THEN システムは所持ステータス切り替えボタンを表示する
4. WHEN 詳細画面で所持ステータスを「所持済み」に変更した時 THEN システムはスプレッドシートの「所持」列をTRUEに更新する
5. WHEN 詳細画面で所持ステータスを「未所持」に変更した時 THEN システムはスプレッドシートの「所持」列をFALSEに更新する
6. WHEN 所持ステータスが更新された時 THEN システムは詳細画面を閉じて一覧画面に戻り、表示リストを更新する
7. WHEN 所持ステータス更新に失敗した時 THEN システムはエラーメッセージを表示し、元の状態に戻す
8. WHEN 所持済みキャラクターを一覧表示する時 THEN システムは異なる背景色やアイコンで区別して表示する

### 要件10: 所持済みキャラクター表示切り替え機能

**ユーザーストーリー:** プレイヤーとして、所持済みキャラクターも含めて全キャラクターを表示したい場合がある。これにより、過去の優先度設定を確認したり、所持ステータスを修正できる。

#### 受入基準

1. WHEN 「所持済みを含む」オプションを有効にした時 THEN システムは所持済みキャラクターも一覧に表示する
2. WHEN 所持済みキャラクターを表示する時 THEN システムは未所持キャラクターと視覚的に区別して表示する
3. WHEN 「未所持のみ」オプションを選択した時 THEN システムは所持済みキャラクターを非表示にする
4. WHEN 表示切り替えを行った時 THEN システムは現在のソート・フィルタ条件を維持する
